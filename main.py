        ...
        async with db_pool.acquire() as conn:
            ...
            if quiz_row:
                ...
                for data in savollar:
                    ...
                    quiz_kwargs = {"chat_id": message.chat.id, "question": q, "options": opts, "type": 'quiz', "correct_option_id": correct, "is_anonymous": False}
...
async def show_profile(message: types.Message):
    ...
    if u: await message.answer(f"👤 **Profilingiz:**\n\nIsm: {u['name']}\n✅ Jami ball: {u['score']}\n📝 Yechilgan testlar: {u['tests_taken']}", parse_mode="Markdown")
    else: await message.answer("Siz hali test yechmadingiz.")

# --- YANGI: RASMDAN LUG'AT TESTI ---
@dp.message(F.text == "📸 Rasmdan lug'at testi")
async def start_vocab_test(message: types.Message, state: FSMContext):
    await state.update_data(msgs_to_delete=[])
...
async def vocab_soni(message: types.Message, state: FSMContext):
    await track_msg(state, message.message_id)
    if not message.text.isdigit(): return
    await state.update_data(soni=int(message.text))
    await state.set_state(VocabForm.vaqt)
    msg = await message.answer("Har bir savol uchun vaqt belgilang:", reply_markup=vaqt_menyu)
    await track_msg(state, msg.message_id)

@dp.message(VocabForm.vaqt)
async def vocab_vaqt(message: types.Message, state: FSMContext):
    await track_msg(state, message.message_id)
    matn = message.text
...
async def vocab_rasm_qabul(message: types.Message, state: FSMContext):
    data = await state.get_data()
    await delete_tracked_msgs(message.chat.id, state)
    ...
    try:
        ...
        FAQAT qat'iy JSON array formatida qaytar: [{{"savol": "'Apple' so'zining tarjimasi nima?", "variantlar": ["Olma", "Nok", "Uzum", "Gilos"], "togri_index": 0}}]"""
        ...
        for data_q in savollar:
            ...
            togri_javob_matni = data_q['variantlar'][eski_index]
            random.shuffle(data_q['variantlar'])
        ...
        await notify_admin("Rasm o'qish xatosi", str(e))
        await wait_msg.delete()
        await message.answer("⚠️ Rasmni o'qishda xatolik yuz berdi. Rasm tiniq ekanligiga ishonch hosil qilib, qaytadan urinib ko'ring.", reply_markup=asosiy_menyu)
        await state.clear()

# --- STANDART TEST YASASH ---
...
async def vaqtni_olish(message: types.Message, state: FSMContext):
    ...
    javob = "Faylni (Word yoki PDF) yuboring." if data['usul'] == 'fayl' else "Mavzuni batafsil yozing:"
...
async def mavzuni_qabul_qilish(message: types.Message, state: FSMContext):
    ...
    prompt = f"'{message.text}' bo'yicha {data['soni']} ta test tuz. Qiyinlik: {daraja_toza}. Variantlar 90 ta belgidan oshmasin. FAQAT JSON ro'yxat ber.\nNamuna: [{{\"savol\": \"...\", \"variantlar\": [\"J1\", \"J2\", \"J3\", \"J4\"], \"togri_index\": 0}}]"
...
async def faylni_qabul_qilish(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not text.strip():
            await wait_msg.delete(); await message.answer("⚠️ Fayldan matn topilmadi.", reply_markup=asosiy_menyu); await state.clear(); return
        daraja_toza = data['daraja'].split()[-2] if "ball" in data['daraja'] else data['daraja'].split()[-1]
        prompt = f"Matn asosida {data['soni']} ta test tuz. Qiyinlik: {daraja_toza}. Variantlar 90 ta belgidan oshmasin. FAQAT JSON ro'yxat ber.\nNamuna: [{{\"savol\": \"...\", \"variantlar\": [\"J1\", \"J2\", \"J3\", \"J4\"], \"togri_index\": 0}}]\n\nMatn: {text[:8000]}"
        await wait_msg.delete()
        ...
        await notify_admin("Fayldan test", str(e)); await wait_msg.delete(); await message.answer("❌ Xatolik yuz berdi.", reply_markup=asosiy_menyu); await state.clear()
...
async def generate_and_save(message: types.Message, prompt: str, wait_msg: types.Message, state: FSMContext, vaqt: int, daraja: str):
    try:
        ...
        inline_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚀 Yechish", url=test_link), InlineKeyboardButton(text="🔗 Ulashish", url=share_link)], [InlineKeyboardButton(text="📥 PDF Yuklash", callback_data=f"pdf_{quiz_id}")]])
        await message.answer("✅ **Test tayyor!**", reply_markup=inline_kb, parse_mode="Markdown"); await state.clear()
    except Exception as e:
        await notify_admin("AI gen", str(e)); await wait_msg.delete(); await message.answer("⚠️ Xatolik.", reply_markup=asosiy_menyu); await state.clear()

@dp.callback_query(F.data.startswith("pdf_"))
async def send_pdf(callback: types.CallbackQuery):
    try:
        quiz_id = callback.data.split("_")[1]
        async with db_pool.acquire() as conn: row = await conn.fetchrow("SELECT savollar FROM quizzes WHERE quiz_id = $1", quiz_id)
        if not row: await callback.answer("⚠️ Topilmadi.", show_alert=True); return
        await callback.answer("🔄 PDF tayyorlanmoqda...", show_alert=False)
        wait_msg = await bot.send_message(callback.message.chat.id, "🔄 Hujjat shakllantirilmoqda...")
        savollar = json.loads(row['savollar']) if isinstance(row['savollar'], str) else row['savollar']
        file_name = f"/tmp/test_{quiz_id}.pdf"
        bot_info = await bot.get_me()
        await asyncio.to_thread(create_pdf_sync, quiz_id, savollar, file_name, bot_info.username)
        await bot.send_document(callback.message.chat.id, FSInputFile(file_name), caption="📥 Marhamat, PDF variant.")
        os.remove(file_name); await wait_msg.delete()
    except Exception as e: await notify_admin("PDF yaratish", str(e)); await bot.send_message(callback.message.chat.id, f"⚠️ Xatolik.")
